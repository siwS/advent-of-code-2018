...
class OverlapDetector
  ...
  def detect_overlaps
    ...
    puts "Number of overlapped inches: #{@count}"
  end
  ...
  private def mark_on_board(starting_x, dimension_x, starting_y, dimension_y)
    ...
    (starting_x..ending_x).each do |i|
      (starting_y..ending_y).each do |j|
        @fabric_array[i][j] += 1
        # Only the first time we detect an overlap for a particular
        # square we increase the count.
        @count += 1 if @fabric_array[i][j] == 2
      end
    end
  end
